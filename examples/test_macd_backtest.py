...
from astock_engine.backtest.backtest_engine import BacktestEngine
...
def generate_trending_data(days=800, symbols=['600519.SH', '000858.SZ', '002475.SZ']):
    """
    生成趋势行情数据（上升趋势+回调）
    适合测试MACD策略
    """
    ...
    for symbol in symbols:
        ...
        # 整体上升趋势
        trend = np.linspace(0, base_price * 0.5, days)  # 50%涨幅
        
        # 中期回调（添加2-3个中期回调）
        t = np.arange(days)
        cycle1 = np.sin(2 * np.pi * t / 80) * base_price * 0.15  # 80天周期
        cycle2 = np.sin(2 * np.pi * t / 120) * base_price * 0.1  # 120天周期
        
        # 随机波动
        noise = np.random.normal(0, base_price * 0.02, days)
        
        # 合成价格
        prices = base_price + trend + cycle1 + cycle2 + noise
        prices = np.maximum(prices, base_price * 0.5)  # 避免价格过低
...
def evaluate_strategy(results: dict) -> dict:
    ...
    # 交易次数（10分）
    if 30 <= num_trades <= 80:  # MACD适中交易频率
        score += 10
    elif 20 <= num_trades < 30 or 80 < num_trades <= 100:
        score += 7
    elif 10 <= num_trades < 20 or 100 < num_trades <= 150:
        score += 5
    elif num_trades >= 10:
        score += 2
    ...
    if win_rate < 0.5:
        suggestions.append("⚠️ 胜率低于50%，需检查入场时机或盈亏比")
    if num_trades < 10:
...
def main():
    print("\n" + "="*60)
    print("MACD趋势策略回测测试".center(60))
    print("="*60 + "\n")
    
    # 1. 生成趋势数据
    print("📊 生成趋势行情数据...")
    data = generate_trending_data(days=800, symbols=['600519.SH', '000858.SZ', '002475.SZ'])
    ...
    plugin = manager.get_plugin('macd_strategy')
    if not plugin:
        print("❌ 未找到MACD策略插件")
        return
    
    print(f"✅ 加载成功: {plugin.metadata.name} v{plugin.metadata.version}")
    print(f"   参数: Fast={plugin.metadata.parameters['fast_period']['default']}, "
          f"Slow={plugin.metadata.parameters['slow_period']['default']}, "
          f"Signal={plugin.metadata.parameters['signal_period']['default']}")
    
    # 3. 创建策略实例
    strategy = manager.create_strategy('macd_strategy')
    
    # 4. 初始化回测引擎
    print("\n🚀 初始化回测引擎...")
    
    from astock_engine.backtest.backtest_engine import BacktestConfig
    ...
    if engine.trades:
        ...
        for i, trade in enumerate(engine.trades[:10], 1):
            ...
            print(f"   {i}. {trade.timestamp.strftime('%Y-%m-%d')}: "
                  f"{action} {trade.symbol} @ {trade.price:.2f} × {trade.quantity}")
