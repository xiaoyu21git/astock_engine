...
DEFAULT_GM_TOKEN = "d307c129700feafb99d344254cb6f88644547c71"
...
class MyQuantBroker(Broker):
    ...
    def place_order(self, req: OrderRequest) -> OrderStatus:
        ...
        # 尝试记录一条实盘动作日志（失败不影响主流程）
        try:  # pragma: no cover - 日志记录失败不应影响交易
        ...
        # 掘金返回的字段名可能包含 cl_ord_id / account_id 等，这里做一些常见字段的兜底映射
        order_id = str(
        ...
        status_text = str(primary.get("status", "NEW"))
    ...
    def query_account(self) -> Dict[str, Any]:
        ...
        try:
            cash_info = get_cash(account_id=self.account_id)  # type: ignore[arg-type]
        except Exception as exc:  # pragma: no cover
    ...
    def get_account_snapshot(self) -> Dict[str, Any]:
        ...
        返回字段示例：
        - total_asset: 账户总资产（优先使用 asset/nav，其次 cash）
        - cash: 账户当前现金或可用资金
        - available: 可用资金（若无明确字段，则与 cash 相同）
        - buying_power: 购买力（若无则为 0.0）
        - raw: 原始返回结构
        """

        info = self.query_account() or {}

        # 某些 gm 版本可能返回 list，这里统一拍平为 dict 视图
